...
static int hyd_stop(struct net_device *d)
{
    netif_stop_queue(d);
    printk("stop %s\n", d->name);
    return 0;
...
static void hyd_uninit(struct net_device *d)
{
    printk("uninit %s\n", d->name);
}

static void hyd_rx(struct sk_buff *skb)
{
...
static netdev_tx_t hyd_xmit(struct sk_buff *skb, struct net_device *d)
{
    ...
    eth = eth_hdr(skb);
    if (eth)
        printk("xmit %s src %pM dst %pM len=%u\n",
               d->name, eth->h_source, eth->h_dest, skb->len);

    payload = skb->data;
    printk("first payload byte data: %02x\n", payload[0]);
...
static long hyd_ioctl(struct file *f, unsigned int c, unsigned long a)
{
    ...
    if (copy_from_user(&req, (void __user *)a, sizeof(req)))
        return -EFAULT;
...
static int __init hyd_init(void)
{
    ...
    if (sysfs_create_file(kobj, &e0.attr))
        printk("sysfs eth0 failed\n");

    if (sysfs_create_file(kobj, &e1.attr))
        printk("sysfs eth1 failed\n");

    printk("Hyd loading Done\n");
    return 0;
}
